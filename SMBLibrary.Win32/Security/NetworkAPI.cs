...
namespace SMBLibrary.Win32.Security
{
    public class NetworkAPI
    {
        ...
        [DllImport("Netapi32.dll")]
        public static extern uint NetUserEnum([MarshalAs(UnmanagedType.LPWStr)] string servername, uint level, uint filter, out IntPtr bufPtr, uint prefmaxlen, out uint entriesread, out uint totalentries, out uint resume_handle);

        // NetUserGetInfo - Retrieves information about a particular user account on a server
        [DllImport("Netapi32.dll")]
        public static extern uint NetUserGetInfo([MarshalAs(UnmanagedType.LPWStr)] string servername, [MarshalAs(UnmanagedType.LPWStr)] string userName, uint level, out IntPtr bufPtr);

        [DllImport("Netapi32.dll")]
        public static extern uint NetUserGetLocalGroups([MarshalAs(UnmanagedType.LPWStr)] string servername, [MarshalAs(UnmanagedType.LPWStr)] string username, uint level, uint flags, out IntPtr bufPtr, uint prefmaxlen, out uint entriesread, out uint totalentries);
        ...
        public static List<string> EnumerateGroups(string userName)
        {
            ...
            {
                throw new Exception("NetUserGetLocalGroups failed, Error code: " + status);
            }
        ...
        public static List<string> EnumerateAllUsers()
        {
            ...
            if (status != NERR_Success)
            {
                throw new Exception("NetUserEnum failed, Error code: " + status);
            }
        ...
        public static bool IsUserExists(string userName)
        {
            ...
            throw new Exception("NetUserGetInfo failed, Error code: " + result);
        }
